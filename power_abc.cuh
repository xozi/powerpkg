...
T* mumuadd(cublasHandle_t handle, 
    ...
    T* temp1;
    T* temp2;
    CUDA_CHECK(cudaMalloc(&temp1, PHASE * sizeof(T)));
    CUDA_CHECK(cudaMalloc(&temp2, PHASE * sizeof(T)));
...
T* m_invert(cublasHandle_t handle, T* var1, T* var2, const int PHASE) {
    // Derference matrices of [a] and [A]
    cuFloatComplex* Aarray[] = { var1 };
    cuFloatComplex* Carray[] = { var2 };

   // Allocate memory for PivotArray and infoArray
    int* PivotArray;
    int* infoArray;
    CUDA_CHECK(cudaMalloc(&PivotArray, PHASE * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&infoArray, sizeof(int)));
...
PowerMatrix<T> matrix_solver(PowerMatrix<T> power) {
    ...
    */
    ...
    // Identity Matrix
    cuFloatComplex u[power.PHASE * power.PHASE] = {
